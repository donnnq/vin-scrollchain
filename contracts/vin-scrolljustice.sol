...
contract VinScrollJustice {
    ...
    function deliverVerdict(uint256 trialId, Verdict verdict, uint256 scrollId, uint256 amount) external onlyArbiter {
        ...
        if (verdict == Verdict.Burned) {
            ...
            emit VerdictDelivered(trialId, verdict, "🔥 Scrolls burned. Judgment executed.");
        } else if (verdict == Verdict.SummonedByDead) {
            emit VerdictDelivered(trialId, verdict, "☠️ Agent summoned by the Dead. Fate unknown.");
        } else {
            emit VerdictDelivered(trialId, verdict, "✅ Agent cleared. No punishment.");
        }
    }

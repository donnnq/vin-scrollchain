// SPDX-License-Identifier: MIT
pragma solidity ^0.8.19;

contract CoalitionLivestreamOverrideCharterDAO {
    address public steward;

    struct CharterEvent {
        string coalitionPartner; // "Greens", "Labor", "Independent"
        string overrideTrigger; // "Blocked Livestream", "Media Ban", "Closed-Door Session"
        string overrideClause; // "Forced Livestream", "Public Archive", "Civil Society Entry"
        string clauseStatus; // "Ratified", "Pending", "Violated"
        string emotionalTag;
        uint256 timestamp;
    }

    CharterEvent[] public events;

    event CharterLogged(
        string coalitionPartner,
        string overrideTrigger,
        string overrideClause,
        string clauseStatus,
        string emotionalTag,
        uint256 timestamp
    );

    modifier onlySteward() {
        require(msg.sender == steward, "Only BatVin may log coalition livestream override rituals");
        _;
    }

    constructor() {
        steward = msg.sender;
    }

    function logCharter(
        string memory coalitionPartner,
        string memory overrideTrigger,
        string memory overrideClause,
        string memory clauseStatus,
        string memory emotionalTag
    ) external onlySteward {
        events.push(CharterEvent({
            coalitionPartner: coalitionPartner,
            overrideTrigger: overrideTrigger,
            overrideClause: overrideClause,
            clauseStatus: clauseStatus,
            emotionalTag: emotionalTag,
            timestamp: block.timestamp
        }));

        emit CharterLogged(coalitionPartner, overrideTrigger, overrideClause, clauseStatus, emotionalTag, block.timestamp);
    }

    function getCharterByIndex(uint256 index) external view returns (
        string memory coalitionPartner,
        string memory overrideTrigger,
        string memory overrideClause,
        string memory clauseStatus,
        string memory emotionalTag,
        uint256 timestamp
    ) {
        require(index < events.length, "Scrollstorm index out of bounds");
        CharterEvent memory c = events[index];
        return (
            c.coalitionPartner,
            c.overrideTrigger,
            c.overrideClause,
            c.clauseStatus,
            c.emotionalTag,
            c.timestamp
        );
    }
}

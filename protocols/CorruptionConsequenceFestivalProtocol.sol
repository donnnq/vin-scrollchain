...
contract CorruptionConsequenceFestivalProtocol {
    address public admin;

    struct FestivalEntry {
        string eventName;
        string consequenceTheme;
        string auditClause;
        string emotionalTag;
        bool summoned;
        bool celebrated;
    ...
    function summonFestival(string memory eventName, string memory consequenceTheme, string memory auditClause, string memory emotionalTag) external onlyAdmin {
        entries.push(FestivalEntry(eventName, consequenceTheme, auditClause, emotionalTag, true, false, false));
    }

...
contract ProxyWarFestivalProtocol {
    address public admin;

    struct FestivalEntry {
        string eventName;
        string featuredConflict;
        string satireTheme;
        string emotionalTag;
        bool summoned;
        bool celebrated;
    ...
    function summonFestival(string memory eventName, string memory featuredConflict, string memory satireTheme, string memory emotionalTag) external onlyAdmin {
        entries.push(FestivalEntry(eventName, featuredConflict, satireTheme, emotionalTag, true, false, false));
    }

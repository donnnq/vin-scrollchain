...
contract EmotionalJournalismFestivalProtocol {
    address public admin;

    struct FestivalEntry {
        string eventName;
        string journalismTheme;
        string civicClause;
        string emotionalTag;
        bool summoned;
        bool celebrated;
    ...
    function summonFestival(string memory eventName, string memory journalismTheme, string memory civicClause, string memory emotionalTag) external onlyAdmin {
        entries.push(FestivalEntry(eventName, journalismTheme, civicClause, emotionalTag, true, false, false));
    }

...
contract SpyCaseFestivalProtocol {
    address public admin;

    struct FestivalEntry {
        string eventName;
        string espionageTheme;
        string reformClause;
        string emotionalTag;
        bool summoned;
        bool celebrated;
    ...
    function summonFestival(string memory eventName, string memory espionageTheme, string memory reformClause, string memory emotionalTag) external onlyAdmin {
        entries.push(FestivalEntry(eventName, espionageTheme, reformClause, emotionalTag, true, false, false));
    }

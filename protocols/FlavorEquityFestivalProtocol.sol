...
contract FlavorEquityFestivalProtocol {
    address public admin;

    struct FestivalEntry {
        string eventName;
        string featuredIngredient;
        string equityTheme;
        string emotionalTag;
        bool summoned;
        bool celebrated;
    ...
    function summonFestival(string memory eventName, string memory featuredIngredient, string memory equityTheme, string memory emotionalTag) external onlyAdmin {
        entries.push(FestivalEntry(eventName, featuredIngredient, equityTheme, emotionalTag, true, false, false));
    }

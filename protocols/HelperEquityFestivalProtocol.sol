...
contract HelperEquityFestivalProtocol {
    address public admin;

    struct FestivalEntry {
        string eventName;
        string logisticsTheme;
        string helperRole;
        string emotionalTag;
        bool summoned;
        bool celebrated;
    ...
    function summonFestival(string memory eventName, string memory logisticsTheme, string memory helperRole, string memory emotionalTag) external onlyAdmin {
        entries.push(FestivalEntry(eventName, logisticsTheme, helperRole, emotionalTag, true, false, false));
    }

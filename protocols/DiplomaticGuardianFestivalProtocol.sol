...
contract DiplomaticGuardianFestivalProtocol {
    address public admin;

    struct FestivalEntry {
        string eventName;
        string guardianTheme;
        string reformClause;
        string emotionalTag;
        bool summoned;
        bool celebrated;
    ...
    function summonFestival(string memory eventName, string memory guardianTheme, string memory reformClause, string memory emotionalTag) external onlyAdmin {
        entries.push(FestivalEntry(eventName, guardianTheme, reformClause, emotionalTag, true, false, false));
    }

...
contract PlanetaryMemoryFestivalProtocol {
    address public admin;

    struct FestivalEntry {
        string eventName;
        string memoryTheme;
        string archivalClause;
        string emotionalTag;
        bool summoned;
        bool celebrated;
        bool sealed;
    ...
    function summonFestival(string memory eventName, string memory memoryTheme, string memory archivalClause, string memory emotionalTag) external onlyAdmin {
        entries.push(FestivalEntry(eventName, memoryTheme, archivalClause, emotionalTag, true, false, false));
    }
